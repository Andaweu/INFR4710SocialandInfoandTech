...
# Get the maximum frequency (highest count).
max_frequency = max(counts.values())

# Find all elements that have the maximum frequency.
most_frequent_elements = [element for element, count in counts.items() if count == max_frequency]

# Print the most frequent element(s) and their frequency.
if most_frequent_elements:
    print(f"The most frequent element(s) is/are: {', '.join(map(str, most_frequent_elements))}")
    print(f"It appears {max_frequency} times.")
else:
